...
if [[ $InstallNumber = 3 ]]
then
pacstrap /mnt base base-devel linux linux-lts linux-headers linux-lts-headers linux-firmware nano xterm networkmanager grub efibootmgr bash-completion xorg-server xorg-apps xorg-xinit xterm xf86-video-amdgpu mesa vulkan-radeon libva-mesa-driver lib32-libva-mesa-driver mesa-vdpau lib32-mesa-vdpau lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings plasma-meta dolphin atom alsa-utils alsa-plugins alsa-oss alsa-firmware sof-firmware alsa-ucm-conf alsa-lib audacity barrier barrier-headless bashtop blender bleachbit bluez bluez-utils chromium codeblocks darktable desmume discord dolphin-emu firefox firejail fish filelight filezilla fuse gimp git gpicview-gtk3 gdk-pixbuf2 jre8-openjdk jre8-openjdk-headless kwallet-pam lutris lightdm-gtk-greeter lightdm-webkit2-greeter mupen64plus nmap notepadqq obs-studio ppsspp python python-py3nvml python-psutil qbittorrent acpi_call-lts screenfetch shotcut steam syncthing tor tree wine whois vlc virtualbox virtualbox-host-dkms virtualbox-guest-iso wireshark-qt wget noto-fonts-emoji ttf-joypixels
fi
